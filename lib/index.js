  ...
  init(config) {
    ...
    return {
      upload(file, customParams = {}) {
        return uploadFile(config, regions, file, file.buffer, S3, customParams);
      },
      uploadStream(file) {
        return uploadFile(config, regions, file, file.stream, S3, customParams);
      },
...
function uploadFile(config, regions, file, buffer, S3, customParams) {
  return new Promise((resolve, reject) => {
